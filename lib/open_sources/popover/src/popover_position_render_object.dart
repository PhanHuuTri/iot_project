...
class PopoverPositionRenderObject extends RenderShiftedBox {
  late PopoverDirection _direction;
  ...
  Offset calculateOffset(Size size) {
    ...
    } else if (_direction == PopoverDirection.bottomLeft ||
        _direction == PopoverDirection.bottomRight) {
      return _dxyOffset(_direction, _verticalOffset(size), size);
    } else {
  ...
  Offset _dxOffset(
    ...
    if (direction == PopoverDirection.bottom) {
      ...
      return Offset(horizontalOffset, max(8.0, attachRect.top - size.height));
    }
  }
  ...
  Offset _dxyOffset(
    ...
    if (_direction == PopoverDirection.bottomRight) {
      return Offset(
          attachRect.bottomRight.dx - attachRect.width, attachRect.bottom);
    } else {
      return Offset(attachRect.bottomRight.dx - size.width, attachRect.bottom);
    }
  }
